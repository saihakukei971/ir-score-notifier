...
## 🔁 IR Impact Notifier - 処理フロー概要

### ① 入力フェーズ  
- ユーザー手動入力（テキスト・URL・CSV）  
- 監視フォルダ内のIRファイル（自動検出）
...
### ④ 通知判定フェーズ  
- 設定された閾値（例：80点）を超えるか判定  
- 超えていればSlack通知へ進行

          ▼

### ⑤ Slack通知フェーズ（該当時のみ）  
- Webhookにより通知（証券コード・見出し・スコア付き）
...
### ⑦ 辞書自動生成（ログと連携）  
- ログ蓄積に基づき頻出語・影響語を自動抽出  
- TF-IDF分析 → 自動辞書（JSON）として保存
